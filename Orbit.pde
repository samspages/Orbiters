...
void recurseUpdateOrbiters(Orbiter node, double timeDelta) {
  ...
  for (int i = 0; i < node.getChildren().size(); i++) {
    recurseUpdateOrbiters(node.getChildren().get(i), timeDelta);
  }
}
...
void updateOrbiters(double timeDelta) {
  ...
  recurseUpdateOrbiters(sun, timeDelta);
}

void recurseDrawOrbiters(Orbiter node) {
  drawOrbiter(node);
  ...
  for (int i = 0; i < node.getChildren().size(); i++) {
    recurseDrawOrbiters(node.getChildren().get(i));
  }
}
